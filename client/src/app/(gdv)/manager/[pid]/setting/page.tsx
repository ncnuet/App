...
const StaffInfor = ({ searchParams }: StaffInforProps) => {
  ...
  const [data, setData] = useState<IDetailStaffInfor | null>(null);
  ...
  const onSubmitAvatar = async () => {
    if (pid !== null && avatar !== null) {
      try {
        const response = await editStaffAvatar(pid, avatar);
      } catch (error) {
        alert(JSON.stringify(error));
      }
    }
